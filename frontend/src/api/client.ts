...
export const getCaratMatrixDashboard = (params?: { shape?: string; cut?: string; polish?: string; symmetry?: string; fluorescence?: string; lab?: string; country?: string; growth_type?: string }) => api.get<{ carat_matrix: CaratMatrixRow[]; color_clarity_matrix: ColorClarityRow[]; range_color_matrix: RangeColorRow[]; range_color_clarity_matrix: RangeColorClarityRow[] }>('/market-intelligence/carat-matrix-dashboard', { params }).then(res => res.data);
export const downloadCaratMatrixExcel = (params: { shape?: string; cut?: string; polish?: string; symmetry?: string; fluorescence?: string; lab?: string; country?: string; growth_type?: string }) => api.get('/market-intelligence/carat-matrix-export', { params, responseType: 'blob' }).then(res => {
  const url = window.URL.createObjectURL(new Blob([res.data]));
  const link = document.createElement('a');
  link.href = url;
